...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        #There are no edges in the given file, only WeightedEdges so
        #you will only need to add WeightedEdges to the Digraph
        
        src = edge.get_source()
        dest = edge.get_destination()
        total = edge.get_total_distance()
        outdoor = edge.get_outdoor_distance()
        WeEd = WeightedEdge(src, dest, total, outdoor)
        if not (src in self.nodes and dest in self.nodes):
            raise ValueError('1 or more missing nodes')
        else:
            self.edges[src].append(WeEd)
